...
In Sheet 1 of this chapter, you were introduced to homomorphisms and isomorphisms. In this sheet, we
will take a deeper dive with morphisms and some attributes that definitionally separate different
kids of morphisms.
...
`AlgebraInLean`, this means that a _group_ endomporphism is a group homomorphism from an arbitrary
group G back to itself.

As you have seen previously, in the context of Algebra, "group" is often omitted when discussing
group endomorphisms. An endomorphism (and morphisms in general) can be defined among many different
types of mathematical objects, but in AlgebraInLean it will always refer to a group endomorphism.
...
existence of the endomorphism mapping some arbitrary vector v ↦ 0, and the identity mapping v ↦ v.
...
/-
An automorphism is defined to be an endomorphism that is also a bijection. You will recognize the
following definition is similar to how we defined bijectivity in the first place.
-/
def Automorphism (φ : G → G) : Prop := Endomorphism φ ∧ Bijective φ
/-

You can think of it like a permutation from a group to itself, although it is important that this
permutation respects the group structure. See more specifically what "respecting the group
structure" looks like in the next chapter (keep an eye out for orders!).

-/

/-
You may be able to intuitively discern that all automorphisms are also isomoprhisms. Let's do a
brief exercise to prove this before jumping into some more complex examples.
-/
...
As another exercise, let's prove that a specific function mapping within the group of integers under
...
endomorphism. We are already defining it as the map φ : G → G (a group onto itself), so it suffices
to prove that φ is a homomorphism. That may be useful going forward with this proof.
