...
namespace Morphisms
  ...
  section Maps
    ...
    example (f : X → Y) (g : Y → Z) (h1: Surjective f) (h2 : Surjective g) : Surjective (g ∘ f) := by
       sorry

    example (f : X → Y) (g : Y → Z) (h1: Injective f) (h2 : Injective g) : Injective (g ∘ f) := by
       sorry

    example (f : X → Y) (g : Y → Z) (h1 : Injective (g ∘ f)) (h2 : Injective f) : Injective g := by
       sorry
  ...
  group_hom φ) (g : G) : φ (ι g) = ι (φ g) := by
