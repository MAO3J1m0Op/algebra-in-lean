...
From the type signature obtained by `#check Nat.gcd`, we can see that the _greatest common
denominator_ function in mathlib takes in two natural numbers and outputs the largest natural number
that will divide both inputs (ℕ → ℕ → ℕ). Particularly, the gcd of any number and 0 is the number
itself.
...
The "naive" way to do it is through _prime factorization_; break up each of the numbers into their
constituent, atomic parts, and then find the largest part they have in common. But this brute-force
algorithm scales very poorly for large numbers, a performance bottleneck that cryptographic schemes
like RSA depend on.
...
Similarly, you might expect a * b (mod m) = (a mod m) * (b mod m). Why do
